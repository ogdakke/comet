...
final class WorkspaceStore {
    ...
    ) async throws -> AsyncThrowingStream<StudioThreadListResponse, Error> {
        try await relay(for: deviceId).stream(
            method: "WatchStudioConversations",
    ...
    func watchStudioThread(
        deviceId: String,
        threadId: String
    ) async throws -> AsyncThrowingStream<StudioThread, Error> {
        try await relay(for: deviceId).stream(
            method: "WatchStudioConversation",
