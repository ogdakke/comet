...
async fn devin_discovery_errors_and_timeouts_retry_without_stale_success() {
    ...
    // Apply the short deadline only to the deliberately wedged CLI. Normal
    // Python startup must not race a 500 ms budget under parallel test load.
    ...
    std::fs::write(dir.path().join("state"), "gpt-new").unwrap();
    let harness = harness.with_model_discovery_timeout(Duration::from_secs(10));
    assert_eq!(harness.models().await.unwrap()[0].id, "gpt-new");
}
