//! Real native Pi RPC + model regression for #296. Requires an authenticated Pi and
//! fixtures/pi-slow-model.ts loaded as a Pi extension (35s delay by default).
//! PI_EXECUTABLE can select an isolated native Pi installation.
            ...
            match event {
                AgentEvent::ToolResult { .. } if tool_at.is_none() => {
                    ...
                    for word in ["SECOND-DONE", "THIRD-DONE"] {
                        steer.send(SteerMessage { prompt: format!("Do not call tools. Reply exactly {word}."), message_id: None, follow_up: true }).await.unwrap();
                    }
                }
