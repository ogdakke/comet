    ...
    fn render_row(&mut self, ix: usize, window: &mut Window, cx: &mut Context<Self>) -> AnyElement {
        ...
        match row {
            ...
            DiffRow::Line {
                ...
                let shaped = (!matches!(code_width, DiffCodeWidth::Wrapped)).then(|| {
                    self.line_layouts.get(layout_key).unwrap_or_else(|| {
                        let mono = font(theme.font_mono.clone());
...
fn code_text_viewport(
    ...
    let content = div()
        ...
        .child(match shaped {
            Some(shaped) if !wrapped => shaped_diff_text(shaped),
            _ => gpui::StyledText::new(text)
                .with_runs(runs)
                .into_any_element(),
        });
    let viewport = div()
