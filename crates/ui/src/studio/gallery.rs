...
const MAX_IN_FLIGHT_IMAGES: usize = 20;
...
impl StudioPage {
    ...
    pub(super) fn request_images(
        ...
        for id in ids {
            if inflight >= MAX_IN_FLIGHT_IMAGES {
                break;
            }
            let ready = if full {
            ...
            if ready || self.loading_images.contains(&id) || self.image_failed.contains(&id) {
                continue;
            }
