...
impl EngineBackend for InProcessEngine {
    ...
    async fn shutdown(&self) {
        ...
        if let Some(ipc) = self.ipc_task.lock().await.take() {
            ipc.abort();
            // Cancellation is scheduled; join before promising that IPC has
            // closed, including when bootstrap fails before stores open.
            let _ = ipc.await;
        }
