...
pub use crate::text_input::{
    CARET_BLINK_MS, DRAG_SCROLL_FRAME_MS, TextInput, caret_visible, input_content_height,
...
pub use crate::text_input::{
    ...
    DeleteWordRight, DocEnd, DocStart, Down, End, Home, Left, MentionTab, Newline, Paste, Redo,
    Right, SelectAll, SelectDocEnd, SelectDocStart, SelectDown, SelectEnd, SelectHome, SelectLeft,
    SelectRight, SelectUp, SelectWordLeft, SelectWordRight, Submit, Undo, Up, WordLeft, WordRight,
};

actions!(composer, [MessageNewlineOrAccept, ModifiedSubmit]);
...
impl ComposerInput {
    ...
    pub fn visible_point_for_index(&self, index: usize) -> Option<Point<Pixels>> {
        self.point_for_index(index).map(|point| Point {
            x: point.x - px(self.scroll_left),
            y: point.y - px(self.scroll_top),
        })
    }

    pub fn set_soft_wrap(&mut self, soft_wrap: bool) {
        if self.wrap_enabled != soft_wrap {
            self.wrap_enabled = soft_wrap;
    ...
    fn up(&mut self, _: &Up, _: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.key_context == MESSAGE_COMPOSER_CONTEXT && self.selected_range == (0..0) {
            cx.emit(ComposerInputEvent::HistoryNavigate(-1));
        } else if let Some(ix) = self.vertical_target(-1.0) {
            self.move_to(ix, cx);
        }
    }
    ...
    fn down(&mut self, _: &Down, _: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.key_context == MESSAGE_COMPOSER_CONTEXT
    ...
    fn layout_text(
        ...
        let lines = window
            ...
            .shape_text(
                ...
                font_size,
                &runs,
                (!self.single_line && self.wrap_enabled).then_some(width),
                None,
            )
...
fn mention_error_message(err: &RpcError) -> SharedString {
    match err {
        ...
        RpcError::BadParams(_) | RpcError::Failed(_) | RpcError::FailedStructured { .. } => {
...
impl Composer {
    ...
    fn render_input_with_completion(&self, _theme: &Theme, _cx: &mut Context<Self>) -> gpui::Div {
        div()
            .relative()
            .h_full()
            .min_h_0()
    ...
    fn render_wizard(&mut self, window: &mut Window, cx: &mut Context<Self>) -> gpui::AnyElement {
        ...
        body_opts.link = Some(crate::markdown::render::LinkUi {
            source_session: None,
            handler: Rc::new(move |activation, window, cx| {
                let url = activation.target.original.as_str();
                        ...
                        crate::markdown::render::heading_index_for_fragment(&headings, &fragment)
                {
                    ...
                    window.refresh();
                    return crate::markdown::render::LinkOutcome::Internal;
                }
...
mod tests {
    ...
    #[gpui::test]
    fn studio_and_search_fields_keep_editor_shortcuts_after_composer_init(
        cx: &mut gpui::TestAppContext,
    ) {
        cx.update(|cx| {
            gpui_base::init(cx);
        ...
        for search in [false, true] {
            let handle = cx.add_window(|window, cx| {
                let mut input = if search {
                    TextInput::palette_search("Search", cx)
                } else {
                    TextInput::composer("Studio prompt", cx)
                };
                input.set_text("replace this draft", cx);
                window.focus(&input.focus_handle(cx), cx);
                input
            });
            cx.update_window(handle.into(), |_, window, cx| window.draw(cx).clear())
                .unwrap();
            cx.simulate_keystrokes(handle.into(), "cmd-a backspace");
            assert_eq!(
                handle
    ...
    #[gpui::test]
    fn recalled_prompt_can_be_undone_to_the_scratch_draft(cx: &mut gpui::TestAppContext) {
        let (_dir, handle) = composer_focus_window(cx);
        handle
            .update(cx, |composer, window, cx| {
                composer.input.update(cx, |input, cx| {
                    input.set_text("unfinished draft", cx);
                    input.replace_from_history("previous prompt", true, cx);
                    assert_eq!(input.selected_range, 0..0);
                    input.undo(&Undo, window, cx);
                    assert_eq!(input.text(), "unfinished draft");
                    input.redo(&Redo, window, cx);
                    assert_eq!(input.text(), "previous prompt");
                });
            })
            .unwrap();
    }
