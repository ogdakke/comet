...
impl Harness for HeldHarness {
    ...
    async fn run(
        ...
        // Publish harness readiness only after its finish receiver exists.
...
async fn queued_text_waits_for_a_steerable_turn_even_with_legacy_policy() {
    ...
    wait_for(
        || prompts.lock().unwrap().iter().any(|m| m == "first queued"),
        "first queued harness ready",
    )
    .await;
...
/// Queue drains notify only for their final turn. Send now also suppresses
/// its interrupted predecessor; the replacement completion must still arrive.
#[tokio::test(flavor = "multi_thread", worker_threads = 2)]
async fn queue_completion_markers_distinguish_normal_turns_from_interrupts() {
    for send_now in [false, true] {
        ...
        assert_eq!(
            ...
            "an internal queue handoff should not notify"
        );
        harness.finish.send(()).unwrap();
        wait_for(
